...
GitHub: https://github.com/TiefSeeTaucher69/ScubaManTakesOff
...
**Migration:** If `HasXItem == 1` and `ItemCount_X == 0` → give 1 free stack via `CloudSaveManager.Instance.SaveInt` (runs once in GameScene `Start()`). Uses cloud save so the migrated stack survives a PlayerPrefs wipe.
...
| `PlayerAccountsLinked` | int | 0/1 — 1 = player has logged in via Unity Player Accounts (not cloud-synced) |
| `MasterVolume` | float | Audio mixer master volume 0–1 (not cloud-synced) |
| `MusicVolume` | float | Audio mixer music volume 0–1 (not cloud-synced) |
| `SFXVolume` | float | Audio mixer SFX volume 0–1 (not cloud-synced) |
...
**Leaderboard IDs:** `SMtakesoffLeaderboard` (Quickplay), `SMtakesoffRankedLeaderboard` (Ranked)
...
- Progress: `CannabisStash`, `Highscore`, `RankedHighscore`, `TotalScore`, `TotalRuns`, `TotalXP`, `PlayerLevel`, `Username`
- Ownership: `HasSkin_*`, `HasTrailRed/Purple/Blue`, `HasPet*`, `HasBiome*`, legacy `HasInvincibleItem` etc.
- Item stacks: `ItemCount_Invincible/Shrink/Laser/SlowMo/Shield`
- Active selections: `ActiveSkin`, `ActiveTrail`, `ActiveItem`, `ActivePet`, `ActiveBiome` — synced via `SaveString` in LoadoutScript/ShopCardScript
- Daily reward: `lastRewardDate`
- Weekly missions: `WeeklyMissions`, `WeeklyMissionStartTime`, `MissionRewardCollected_*`

**NOT synced (device-specific, raw `PlayerPrefs` only):** `VSyncEnabled`, `FPSCap`, `ResolutionIndex`, `MasterVolume`, `MusicVolume`, `SFXVolume`, `PlayerAccountsLinked`
